...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    {
        pr_err("Inconsistent firmware size, Update aborted! Default size: %d(%dK), actual size: %d(%dK)", fw_len, fw_len/1024, update_msg.fw_total_len, update_msg.fw_total_len/1024);
        return FAIL;
    }
	pr_info("Firmware length:%d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
					...
					u16 start_addr, u8 bank_cmd)
{
	...
	ret = gup_recall_check(client, fw_section, start_addr,
							FW_SECTION_LENGTH);
	if(FAIL == ret) {
		pr_err("[burn_fw_section]recall check %dk firmware fail.", FW_SECTION_LENGTH/1024);
		return FAIL;
	}
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		fw_dsp_isp = (u8*)kzalloc(FW_DSP_ISP_LENGTH,
								GFP_KERNEL);
		if (fw_dsp_isp == NULL) {
			continue;
		} else {
...
ret = gup_set_ic_msg(client, _bWO_MISCTL__CPU_SWRST_PULSE, 0x01);
	...
	ret = gup_set_ic_msg(client, _bRW_MISCTL__MEM_CD_EN, 0x01);
	if (ret <= 0) {
		pr_debug("[burn_dsp_isp]enable accessing code fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	pr_debug("[burn_dsp_isp]Burned length:%d", update_msg.fw_burned_len);
	ret = SUCCESS;
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		fw_ss51 = (u8*)kzalloc(FW_SECTION_LENGTH,
							GFP_KERNEL);
		if (fw_ss51 == NULL) {
			continue;
		} else {
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		fw_dsp = (u8*)kzalloc(FW_DSP_LENGTH,
							GFP_KERNEL);
		if (fw_dsp == NULL) {
			continue;
		} else  {
	...
	pr_debug("[burn_fw_dsp]Burned length:%d", update_msg.fw_burned_len);
	ret = SUCCESS;
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		fw_boot = (u8*)kzalloc(FW_BOOT_LENGTH,
							GFP_KERNEL);
		if (fw_boot == NULL) {
			continue;
		} else {
	...
	pr_debug("[burn_fw_boot]step6:burn 2k bootloader firmware");
...
s32 gup_update_proc(void *dir)
{
	...
	while (retry++ < 5) {
		...
		if (FAIL == ret) {
			pr_err("[update_proc]burn bootloader firmware fail.");
			continue;
	}
		show_len = 50;
